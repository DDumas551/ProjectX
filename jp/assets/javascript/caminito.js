...
$(document).ready(function(){
    ...
    function trailMap(data){
        ...
        //  var queryURLdirection = "https://cors-anywhere.herokuapp.com/https://maps.googleapis.com/maps/api/directions/js?origin=SanFrancisco&destination=Berkeley"
        //  $.ajax({
        //     url: queryURLdirection,
        //     method: "GET",
        //     headers: {
        //         "Authorization": "Bearer AIzaSyAIbmWFTKZvx7_IcqVW9iSvh5g91vvdIvY"
        //     }, 
        //     dataType: "json",
        //  })
                ...
                success: function(response){
                    ...
                    for(let i = 0; i < response.businesses.length; i++){
                        $("#trailmix").append("<tr><td>" + response.businesses[i].name + "</td><td>" + response.businesses[i].location.address1 + "</td><td>"
                         + Math.floor(response.businesses[i].distance * .0006)  + " " + "miles" + "</td><td>" + response.businesses[i].rating + 
                         "</td><td><img src='" + response.businesses[i].image_url  + "'/></td>"  + "</tr>")
            ...
            $.ajax({
                ...
                success: function(response){
                    ...
                    for(let i = 0; i < response.businesses.length; i++){
                        ...
                        //markers.push(marker);
                        attachMessage(marker,response.businesses[i].name);
                    ...
                    function attachMessage(marker, content){
                        ...
                        marker.addListener('click',function() {
                            ...
                            var queryURLdirection = "https://cors-anywhere.herokuapp.com/https://maps.googleapis.com/maps/api/directions/json?origin=Berkley&destination=" + content + "&key=AIzaSyAIbmWFTKZvx7_IcqVW9iSvh5g91vvdIvY"
                            $.ajax({
                                url: queryURLdirection,
                                method: "GET",
                                headers: {
                                ...
                                dataType: "json",
                                success: function(response){
                                    let data = response.routes[0].legs[0].steps;
                                    console.log(data)
                                    for(let i = 0; i < data.length; i++){
                                        $("#direction").append("<tr><td>" + data[i].html_instructions + "</td><td>" + data[i].duration.text + "</td><td>" + data[i].distance.text + "</td></tr>")

                                        console.log(data[i].duration)
                                        console.log(data[i].distance)
                                        console.log(data[i].html_instructions);
                                    }
                                }
                            })
                        });
    ...
    $.ajax({
      ...
      .then(function(response) {
        ...
        // Transfer content to HTML
        $(".city").html("<h1 id='currentWeather'>" + response.name + " Current Weather Details</h1>");
    ...
    $.ajax({
      ...
      .then(function(response1) {
        ...
        //"</td><td>" + response1.list[i].weather[i].description  + 
      ...
      var queryURL2 =
      "https://api.openweathermap.org/data/2.5/uvi/forecast?lat=37.87078378632823&lon=-122.30097999999998"   + "&units=imperial&appid=" +
    APIKey;
  ...
  $.ajax({
     ...
     $.ajax({
        url: "https://trailapi-trailapi.p.mashape.com/trails/explore/?lat=37.87078378632823&lon=-122.30097999999998>",
        headers: {"X-Mashape-Key":"hbQJjkFVPkmshcm3vcER6xs4j8UEp1JbkPhjsnV6cHWR3H5MDK", "Accept": "application/json"},
    ...
    //  var APIKey ="hbQJjkFVPkmshcm3vcER6xs4j8UEp1JbkPhjsnV6cHWR3H5MDK"

    //  var queryURL =
    //     "https://trailapi-trailapi.p.mashape.com/trails/www.trailapi.com" 
